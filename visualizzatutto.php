...
<body leftmargin="0" topmargin="0">
	...
	<?php
		...
		$q= "	SELECT * 
				FROM rielaborati
				;";
                $q1=" SELECT round((re/cp*100),2) , round((ro/ti*100),2), round((ro/rv*100),2), round((rv/ti),2), round((ti/cp),2) , round((re/ro),2)
FROM rielaborati;";
		$query= $con->query($q);
		$num_risultati= $query->num_rows;
                $query1= $con->query($q1);
		$i= 0;
                while ($i<$num_risultati)
                {		
			...
			$id= $riga["id"];
                    ...
                    $riga1= $query1->fetch_assoc();
			
			$ROE= $riga1["round((re/cp*100),2)"];
                        $ROI= $riga1["round((ro/ti*100),2)"];
                        $ROS= $riga1["round((ro/rv*100),2)"];
                        $ROT= $riga1["round((rv/ti),2)"];
                        $LV= $riga1["round((ti/cp),2)"];
                        $IN= $riga1["round((re/ro),2)"];
		...
		echo "            <label>ID: </label><td><b>$id</b></td>
			...
			echo "	<tr>
						...
						<td>$ROI</td>
                         ...
                         $i= $i + 1;
		echo "</table><br> <a href=\"modifica.php?id=$id&r_s=$r_s&anno=$anno&ac=$ac&dl=$dl&df=$df&im=$im&db=$db&dc=$dc&cp=$cp&re=$re&ro=$ro&rv=$rv&ti=$ti\">Modifica</a>
                
                <a href=\"elimina.php?id=$id\">Elimina</a> <br><br><hr><br> ";}              
